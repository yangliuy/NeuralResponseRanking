...
public class MatrixUtil {
	...
	public static void norm2(Double[] thetaD) {
		double norm2Value = 0;
		for(int i = 0; i < thetaD.length; i++){
			norm2Value += Math.pow(thetaD[i], 2);
		}
		for (int i = 0; i < thetaD.length; i++)
			thetaD[i] = thetaD[i] / Math.sqrt(norm2Value);
	}

	public static double dotProdNormalize(Double[] queryWordVec, Double[] answerWordVec) {
		// TODO Auto-generated method stub
		norm2(queryWordVec);
		norm2(answerWordVec);
		double res = 0;
		//System.out.println("queryWordVec.length: " + queryWordVec.length);
		for(int i = 0; i < queryWordVec.length; i++){
			res += queryWordVec[i] * answerWordVec[i];
		}
		return res;
	}
}
