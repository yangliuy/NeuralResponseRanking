...
KD means integrate the qa_cooccur_matrix into the (context, response) matching process
...
class DMN_CNN_KD(BasicModel):
    def __init__(self, config):
        super(DMN_CNN_KD, self).__init__(config)
        self.__name = 'DMN_CNN_KD'
        ...
        self.i = 0
        if not self.check():
            raise TypeError('[DMN_CNN_KD] parameter check wrong')
        print '[DMN_CNN_KD] init done'
    ...
    def build(self):
        ...
        for i in range(self.config['text1_max_utt_num']):
            ...
            # try to add cross4 on indicator matrix
            # try to add cross5 on cosine matrix
            # try to add cross6 on bi-linear matrix
