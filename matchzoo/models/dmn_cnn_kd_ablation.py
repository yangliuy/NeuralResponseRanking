# -*- coding=utf-8 -*-
'''
Implementation of DMN-KD for conversational response ranking in information-seeking conversation
Model ablation analysis for DMN-KD model
...
class DMN_CNN_KD_ABLATION(BasicModel):
    def __init__(self, config):
        super(DMN_CNN_KD_ABLATION, self).__init__(config)
        self.__name = 'DMN_CNN_KD_ABLATION'
        ...
        self.i = 0
        if not self.check():
            raise TypeError('[DMN_CNN_KD_ABLATION] parameter check wrong')
        print '[DMN_CNN_KD_ABLATION] init done'
    ...
    def build(self):
        ...
        def slice_cur_utt(x):
            x1 = K.tf.slice(x, [0, self.i, 0, 0], [-1, 1, self.config['text1_maxlen'], self.config['text2_maxlen']])
            return x1
        ...
        qa_comat = Input(name='qa_comat', shape=(self.config['text1_max_utt_num'], self.config['text1_maxlen'],self.config['text2_maxlen'],))
        ...
        for i in range(self.config['text1_max_utt_num']):
            ...
            if cross_matrix != None and inter_type != None:
                ...
                elif cross_matrix == 'm3': # only m3
                    cross3 = Lambda(slice_cur_utt)(qa_comat)  # qa co-occur matrix
                    cross3_reshape = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 1))(cross3)
                elif cross_matrix == 'm1m3': # m1+m3
                    cross1 = Match(match_type='dot')([q_embed, d_embed])  # dot product of embeddings
                    cross3 = Lambda(slice_cur_utt)(qa_comat)  # qa co-occur matrix
                    cross3_reshape = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 1))(cross3)
                elif cross_matrix == 'm2m3': # m2+m3
                    cross2 = Match(match_type='dot')([q_rep, d_rep])  # dot product of GRU output representations
                    cross3 = Lambda(slice_cur_utt)(qa_comat)  # qa co-occur matrix
                    cross3_reshape = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 1))(cross3)
                else:
                ...
                # inter_type will not change cross3
                cross3 = Lambda(slice_cur_utt)(qa_comat)  # qa co-occur matrix
                cross3_reshape = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 1))(cross3)
                if inter_type == 'dot':
                    cross1 = Match(match_type='dot')([q_embed, d_embed])  # dot product of embeddings
                    ...
                    cross2 = Match(normalize=True, match_type='dot')(
                        [q_rep, d_rep])  # cosine product of GRU output representations
            ...
            if cross3_reshape != None:
                show_layer_info('QA_Comat-cross3-reshape', cross3_reshape)

            # concat if the number of not-None matrix >= 2
            not_none_matrix_number = 0
            if cross1 != None:
                not_none_matrix_number += 1
            if cross2 != None:
                not_none_matrix_number += 1
            if cross3_reshape != None:
                not_none_matrix_number += 1

            if not_none_matrix_number > 1: # need concat
                if cross1 != None and cross2 != None and cross3_reshape != None:
                    cross = Lambda(concate)([cross1, cross2, cross3_reshape])
                    z = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 3))(cross) # batch_size * t1_len * t2_len * 3 channels
                elif cross1 != None and cross3_reshape != None:
                    cross = Lambda(concate)([cross1, cross3_reshape])
                    z = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 2))(cross)
                elif cross2 != None and cross3_reshape != None:
                    cross = Lambda(concate)([cross2, cross3_reshape])
                    z = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 2))(cross)
                else:
                    raise TypeError('[DMN_CNN_ABLATION] the concat type is not correct!')
            elif not_none_matrix_number == 1: # no need concat
                if cross1 != None:
                    z = cross1
                elif cross2 != None:
                    z = cross2
                ...
                raise TypeError('[DMN_CNN_ABLATION] all of matrics are None!')
        ...
        model = Model(inputs=[query, doc, qa_comat], outputs=out_)
        return model
