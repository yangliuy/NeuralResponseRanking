...
mz_specialized_losses = {'rank_hinge_loss', 'rank_crossentropy_loss', 'rank_crossentropy_loss2'}
...
# def rank_crossentropy_loss2(kwargs=None):
#     neg_num = 1
#     if isinstance(kwargs, dict) and 'neg_num' in kwargs:
#         neg_num = kwargs['neg_num']
#     def _cross_entropy_loss(y_true, y_pred):
...
#         y_pos_logits = Lambda(lambda a: a[::(neg_num+1), :], output_shape= (-1,2))(y_pred)
#         y_pos_labels = Lambda(lambda a: a[::(neg_num+1), :], output_shape= (1,))(y_true)
#         print('y_pos_logits: ', y_pos_logits.get_shape().as_list())
#         print('y_pos_labels: ', y_pos_labels.get_shape().as_list())
#         logits_list, labels_list = [y_pos_logits], [y_pos_labels]
#         for i in range(neg_num):
#             y_neg_logits = Lambda(lambda a: a[(i+1)::(neg_num+1), :], output_shape= (-1,2))(y_pred)
#             y_neg_labels = Lambda(lambda a: a[(i+1)::(neg_num+1), :], output_shape= (1,))(y_true)
#             logits_list.append(y_neg_logits)
#             labels_list.append(y_neg_labels)
#         print('logits_list: ', len(logits_list))
...
#         return tf.reduce_mean(sparse_categorical_crossentropy(labels, logits))
#     return _cross_entropy_loss
